...
with open(r'C:\Users\Lenovo\Desktop\telegram bot\bot.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Find function start
start_idx = None
for i, line in enumerate(lines):
    if 'async def handle_video_button' in line:
        start_idx = i
        break

# Find function end (next async def)
end_idx = None
for i in range(start_idx + 1, len(lines)):
    if lines[i].startswith('async def handle_profile'):
        end_idx = i
        break

new_func = '''async def handle_video_button(
        update: Update,
...
with open(r'C:\Users\Lenovo\Desktop\telegram bot\bot.py', 'w', encoding='utf-8') as f:
    f.writelines(new_lines)

print(f'Function replaced: lines {start_idx+1} to {end_idx}')
